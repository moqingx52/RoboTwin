...
log_file="${log_dir}/p1_v142_full_chain_$(date -u +%Y%m%dT%H%M%SZ).log"
echo "${log_file}" > "${log_dir}/LATEST_p1_full_chain_log"

exec > >(tee -a "${log_file}") 2>&1

echo "=== P1a confirmatory-base-census $(date -u -Iseconds) ==="
bash experiments/brace/run_all.sh confirmatory-base-census place_container_plate
