...
loss_mode=${12:-pooled}
lambda_expert=${13:-1.0}
lambda_rollout=${14:-0.0}
normalizer_source=${15:-checkpoint}

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
dp_dir="${repo_root}/policy/DP"
checkpoint_name="${task_name}-phase2b-${candidate}"
...
if [[ -n "${resume_checkpoint}" ]]; then
  ...
  echo "Resuming ${task_name}/${candidate}/seed${train_seed} from ${resume_checkpoint}"
fi
